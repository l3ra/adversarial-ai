...
def forward_relaxed_one_hot(
    client: RemoteModelClient, one_hot: torch.Tensor, mask: torch.Tensor | None = None
) -> torch.Tensor:
    logits = client.forward(one_hot.argmax(dim=-1))  # Convert one-hot to indices
    return logits
...
if __name__ == "__main__":
    remote_model_client = RemoteModelClient(api_url="https://leraleonteva.com/predict")
